...
class Scout(Person):
    birth_certificate_number = models.CharField(_('Birth Certificate Number'), max_length=8, null=True, blank=True)
    section = models.CharField(_('section'), max_length=10, choices=SECTION, db_index=True)
    unit = models.ForeignKey(Unit, on_delete=models.PROTECT, db_index=True)
    investiture = models.BooleanField(_('investiture'), default=False, db_index=True)
    jasiri_investiture = models.BooleanField(_('jasiri investiture'), default=False, db_index=True)
    ...
    chui_badge_award = models.BooleanField(_('Chui Badge Award'), default=False, db_index=True)
    ...
    simba_badge_award = models.BooleanField(_('Simba Badge Award'), default=False, db_index=True)
    csa_award = models.BooleanField('CSA Award', default=False, db_index=True)
    date_of_birth = models.DateField()
    national_id = models.CharField(_('National ID/Passport'), max_length=8, unique=True, null=True, blank=True,
                                   help_text='Leave blank if the Scout is not a Jasiri')
    email = models.EmailField(_('email'), unique=True, null=True, blank=True,
                              help_text=_('example@gmail.com (Leave blank if the Scout is not a Jasiri)'))
    phone_number = models.CharField(_('phone number'), validators=[mobile_num_regex], max_length=13, null=True,
                                    blank=True, help_text='Leave blank if the Scout is not a Jasiri')
    ...
    def clean(self):
        if self.section == 'Jasiri' and not self.national_id:
            raise ValidationError(_('For a Jasiri Scout the National Id/Passport must be provided.'))
        if self.section == 'Jasiri' and not self.email:
            raise ValidationError(_('For a Jasiri Scout the Email must be provided.'))
        if self.section == 'Jasiri' and not self.phone_number:
            raise ValidationError(_('For a Jasiri Scout the Phone Number must be provided.'))
        if self.section == 'Sungura' and self.chui_badge_award:
            raise ValidationError(_('A Sungura Scout cannot attain the highest badge of a Chipukizi Scout.'))
        if self.section == 'Sungura' and self.simba_badge_award:
            raise ValidationError(_('A Sungura Scout cannot attain the highest badge of a Mwamba Scout.'))
        if self.section == 'Sungura' and self.csa_award:
            raise ValidationError(_('A Sungura Scout cannot attain the highest badge of a Jasiri Scout.'))
        if self.section == 'Chipukizi' and self.simba_badge_award:
            raise ValidationError(_('A Chipukizi Scout cannot attain the highest badge of a Mwamba Scout.'))
        if self.section == 'Chipukizi' and self.csa_award:
            raise ValidationError(_('A Chipukizi Scout cannot attain the highest badge of a Jasiri Scout.'))
        if self.section == 'Mwamba' and self.csa_award:
            raise ValidationError(_('A Mwamba Scout cannot attain the highest badge of a Jasiri Scout.'))
        if self.section != 'Jasiri' and self.jasiri_investiture:
            raise ValidationError(_('Only Jasiri Scouts can undergo Jasiri Investiture'))
